...
# Path to ffmpeg executable (assumes ffmpeg is installed globally on macOS)
FFMPEG_PATH = os.path.abspath("bin/ffmpeg")  # MacOS should have ffmpeg available in the PATH
...
def browse_source_video():
    filename = filedialog.askopenfilename(filetypes=[("All Video Files", "*.*")])
...
def upload_to_giphy():
    ...
    if not os.path.exists(output_path):
        messagebox.showerror("Error", "No GIF found. Convert a video first!")
        return

    # Open GIPHY upload page
    webbrowser.open("https://giphy.com/upload")

    # Reveal the GIF in Finder so the user can drag it in
    subprocess.run(["open", "-R", output_path])

root = tk.Tk()
root.title("anyGIF Pro - Video to GIF Converter")
...
tk.Button(root, text="Convert to GIF", command=convert_to_gif, fg="black", bg="#4CAF50").grid(row=6, column=1, pady=10)

tk.Button(root, text="Upload to GIPHY", command=upload_to_giphy, fg="black", bg="#1DA1F2").grid(row=10, column=1, pady=10)

progress_bar = ttk.Progressbar(root, orient="horizontal", length=300, mode="determinate")
progress_bar.grid(row=8, column=1, pady=10)
